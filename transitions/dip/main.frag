...
void main() {
    ...
    // Fully out by the midpoint, then in — the dip colour covers the frame at p = 0.5.
    float fadeOut = clamp(1.0 - u_progress * 2.0, 0.0, 1.0);
    float fadeIn = clamp(u_progress * 2.0 - 1.0, 0.0, 1.0);
    fragColor = u_progress < 0.5 ? mix(dip, a, fadeOut) : mix(dip, b, fadeIn);
}
