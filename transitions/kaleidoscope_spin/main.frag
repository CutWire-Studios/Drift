...
void main() {
    ...
    // Fold in and back out, so both ends of the transition are unmirrored.
    float amt = sin(u_progress * 3.14159265);
    float seg = TAU / max(floor(segments), 2.0);

    float folded = mod(ang + u_progress * spin * TAU, seg);
    folded = abs(folded - seg * 0.5);
    float a2 = mix(ang, folded, amt);

    float rr = r * (1.0 - zoom * amt);
    vec2 suv = 0.5 + vec2(cos(a2), sin(a2)) * rr / vec2(aspect, 1.0);

    fragColor = mix(texture(u_fromTexture, suv), texture(u_toTexture, suv),
