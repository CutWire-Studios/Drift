...
void main() {
    ...
    float amt = sin(u_progress * 3.14159265) * strength;
    vec4 a = texture(u_fromTexture, uv + w * amt);
    vec4 b = texture(u_toTexture, uv - w * amt * 0.6);

    // Heavily-warped regions hand over to B first, so the incoming clip bleeds through.
    float m = smoothstep(0.2, 0.85, u_progress + (length(w) - 0.3) * bleed);
