...
void main() {
    vec2 uv = v_texCoord;
    vec4 a = texture(u_fromTexture, uv);
    vec4 b = texture(u_toTexture, uv);

    float l = dot(a.rgb, vec3(0.2126, 0.7152, 0.0722));
    if (invert > 0.5) l = 1.0 - l;

    // Threshold sweeps past the whole luma range, so dark pixels hand over first.
    float th = mix(-softness, 1.0 + softness, u_progress);
    float m = 1.0 - smoothstep(th, th + softness, l);

    fragColor = mix(a, b, clamp(m, 0.0, 1.0));
}
