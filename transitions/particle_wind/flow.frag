...
// rg = flow vector (biased to 0..1), b = fine grain used for the dissolve threshold.
void main() {
    vec2 uv = v_texCoord;
    float n1 = fbm(uv * 6.0 + vec2(u_progress * 1.5, 0.0));
    float n2 = fbm(uv * 6.0 + vec2(5.2, 1.3) + vec2(0.0, u_progress * 1.5));
    float g = fbm(uv * grain);
    fragColor = vec4(n1, n2, g, 1.0);
}
