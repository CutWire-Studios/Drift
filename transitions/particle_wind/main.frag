...
void main() {
    vec2 uv = v_texCoord;
    vec4 fl = texture(u_currentTexture, uv);
    vec2 flow = (fl.rg - 0.5) * 2.0;
    float g = fl.b;

    float rad = radians(direction);
    vec2 wind = vec2(cos(rad), sin(rad));

    // Displacement ramps quadratically so grains start still and then tear away.
    float t = u_progress * u_progress;
    vec2 off = (wind + flow * turbulence) * t * strength * 0.6;
    off /= vec2(aspectRatio(), 1.0);

    vec4 a = texture(u_fromTexture, uv - off);
    vec4 b = texture(u_toTexture, uv);

    // Grains blow away in an order set by the noise, not by time.
    float lead = dot(uv - 0.5, wind) * 0.5 + 0.5;
    float th = u_progress * (1.0 + spread) - (g * 0.7 + lead * 0.3) * spread;
    a.a *= 1.0 - smoothstep(0.0, 0.3, th);
    a.rgb += vec3(smoothstep(0.8, 1.0, g)) * a.a * 0.12;
