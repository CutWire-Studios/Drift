...
void main() {
    ...
    // Block size peaks at the midpoint and returns to full resolution at both ends.
    float amt = sin(u_progress * 3.14159265) * blockiness;
    float cols = mix(u_resolution.x, 6.0, amt);
    vec2 grid = vec2(cols, max(cols / aspect, 1.0));
    vec2 quv = (floor(uv * grid) + 0.5) / grid;

    vec2 suv = mix(uv, quv, amt);
    vec4 a = texture(u_fromTexture, suv);
    vec4 b = texture(u_toTexture, suv);
    vec4 c = mix(a, b, smoothstep(0.35, 0.65, u_progress));

    // Per-block channel rotation while the blocks are large.
    float h = hash21(floor(uv * grid));
    float swap = amt * colorSwap * step(0.35, h);
    c.rgb = mix(c.rgb, c.gbr, swap);
