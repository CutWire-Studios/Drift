...
void main() {
    ...
    // A slides out to the left, B follows it in from the right.
    vec2 uvA = uv + vec2(u_progress, 0.0);
    vec2 uvB = uv - vec2(1.0 - u_progress, 0.0);

    vec4 a = (uvA.x <= 1.0) ? texture(u_fromTexture, uvA) : vec4(0.0);
    vec4 b = (uvB.x >= 0.0) ? texture(u_toTexture, uvB) : vec4(0.0);
    a.a *= 1.0 - u_progress;
    b.a *= u_progress;
