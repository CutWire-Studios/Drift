...
// Blurs whatever is bound to input 0, so both sides reuse this same pass.
void main() {
    vec2 uv = v_texCoord;
    vec2 c = vec2(centerX, centerY);
    float amt = sin(u_progress * 3.14159265) * strength * 0.25;
    vec2 d = uv - c;

    vec4 acc = vec4(0.0);
    for (int i = 0; i < 16; ++i) {
        float s = float(i) / 15.0;
        acc += texture(u_currentTexture, uv - d * s * amt);
    }
    fragColor = acc / 16.0;
}
