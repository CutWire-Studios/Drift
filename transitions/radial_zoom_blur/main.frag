...
uniform sampler2D u_texture2; // blurred A
uniform sampler2D u_texture3; // blurred B

void main() {
    vec2 uv = v_texCoord;
    float amt = sin(u_progress * 3.14159265);

    vec4 a = mix(texture(u_fromTexture, uv), texture(u_texture2, uv), amt);
    vec4 b = mix(texture(u_toTexture, uv), texture(u_texture3, uv), amt);
    fragColor = mix(a, b, smoothstep(0.4, 0.6, u_progress));
}
